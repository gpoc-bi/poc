...
GCP_BUCKET = "gs://bi-gcp-poc-bucket/"
    ...
    for name in schemas.fieldNames():
        if (dfschema[name] != schemas[name].dataType.typeName()):
            if (schemas[name].dataType.typeName() == "timestamp"):
                dataframe = dataframe.withColumn(name, to_timestamp(name, "yyyyMMdd HH:mm:ss"))
...
def process_dimension_files(df, surrogate_seed, columns):
    df_with_part = df.repartition(1).withColumn(columns[0], monotonically_increasing_id() + surrogate_seed)
...
#
def process_fact_device_unlock(df, df_cust, df_rsn, columns):
    # processing fact
    output = df.join(df_cust.select(['ACCOUNT_NUM', 'DEV_UNLCK_CUST_ID']), ['ACCOUNT_NUM'], 'inner').join(
        df_rsn.select(['ACCOUNT_NUM', 'UNLOCK_STATUS_RSN_ID', 'IMEI', 'DATE_TIMESTAMP']),
        ['ACCOUNT_NUM', 'IMEI', 'DATE_TIMESTAMP'], 'inner')
...
def process_device_unlocking_el(ol_filename, device_unlock_config, fileDate):
    ...
    rt_val_cust[0].write.format('bigquery') \
        .option('table', 'MIRPROD.T_DIM_DEVICE_UNLOCK_CUST') \
    ...
    rt_val_rsn[0].write.format('bigquery') \
        .option('table', 'MIRPROD.T_DIM_DEV_UNLCK_STATUS_RSN') \
        .option("temporaryGcsBucket", "bi-gcp-poc-bucket") \
        .mode('append') \
        .save()

    # Processing fact_device_unlock
    fact_out = process_fact_device_unlock(df, rt_val_cust[0], rt_val_rsn[0],
                                          schemas.device_unlock_fact_schema.fieldNames())
    print("T_FACT_DEVICE_UNLOCK")
    print(fact_out.show())
    fact_out.write.format('bigquery') \
        .option('table', 'MIRPROD.T_FACT_DEVICE_UNLOCK') \
        .option("temporaryGcsBucket", "bi-gcp-poc-bucket") \
        .mode('append') \
        .save()
...
def main():
    ...
    try:
      ...
      device_unlock_config = {"last_processed": "20200414", "max_status_rsn_id": 2160, "max_cust_id": 2160}
      print("hello")
    except:
        e = sys.exc_info()[0]
       # log.error(e)
    ...
    # last processed date
    last_processed_date = datetime.datetime.strptime('20200414', '%Y%m%d').date()
