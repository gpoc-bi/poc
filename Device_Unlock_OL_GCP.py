...
def process_device_unlocking_ol(input_filename, output_filename, curr_processed_date):
   ...
   #output_file.toPandas().to_csv(GCP_BUCKET + output_filename, index=False, header=False, sep=",", date_format="%Y%m%d %H:%M:%S")
    ...
    output_file.write.format('bigquery').option('table', 'OL_AMDOCS_PROD.T_DEVICE_UNLOCK').option("temporaryGcsBucket", "device_unlock_inbound").mode('append').save()
...
def main():
    ...
    CONFIG_FILE1 = GCP_BUCKET + "last_processed_date_ol.txt"
