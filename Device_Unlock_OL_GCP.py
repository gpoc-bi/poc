...
def process_device_unlocking_ol(input_filename, output_filename, curr_processed_date):
   ...
   #output_file.toPandas().to_csv(GCP_BUCKET + output_filename, index=False, header=False, sep=",", date_format="%Y%m%d %H:%M:%S")
    output_file.write.csv(GCP_BUCKET + output_filename)
    output_file.write.format('bigquery').option('table', 'OL_AMDOCS_PROD.T_DEVICE_UNLOCK').option("temporaryGcsBucket", "bi-gcp-poc-bucket").mode('append').save()
...
def main():
    ...
    CONFIG_FILE = GCP_BUCKET + "last_processed_date_ol.txt"
    ...
    if (last_processed_date < date.today()):
            ...
            config_file.repartition(1).write.csv(CONFIG_FILE,header=False,mode = 'overwrite')
                #output_file.write.csv(GCP_BUCKET + output_filename)
